...
public String getGTS(Document xml){      //Analyse de AGG.xml 
       system=""; 
       graphe="";
       regle="";
       typegraphe="";
...
system+=("\n      *******The type of node,edge and Graph in GTS******* \n");
       ...
       for (int i = 0; i < NodeType.size();i++){   //Propriétés des noeuds
       ...
       for (int i= 0; i < EdgeType.size(); i++){     //Propriétés des arcs
	...
	for (int i = 0; i < NodeTypeGraph.size(); i++)
       {
	    	...
	    	for (int j = 0;j <NodeType.size() ; j++){
                                   Element Node = (Element) NodeType.get(j);
                                        if(Nodetgra.getAttributeValue("type").equalsIgnoreCase(Node.getAttributeValue("ID")))
                                          {  typegraphe+=("\nTYP node  :" + Nodetgra.getAttributeValue("type").
				             replace(Nodetgra.getAttributeValue("type"), Node.getAttributeValue("name")));
        ...
        estg=EdgeTypeGraph.size();    //les arcs utilisés dans le graphe source;
	...
	for (int i = 0; i < NodeGraph.size(); i++){
	        ...
	        for (int j = 0;j <NodeType.size() ; j++){
                       ...
                       if(Nodegra.getAttributeValue("type").equalsIgnoreCase(Node.getAttributeValue("ID")))
                         {
                           graphe+=("\nTYP node  :" + Nodegra.getAttributeValue("type").replace(Nodegra.getAttributeValue("type"),
...
regle+=("\n voici les arcs des règles "+ graphrule.getAttributeValue("name")+"\n");
siezeeLHS[i]= Edgepartrulle.size();    
                ...
                for (int k=0;k < Edgepartrulle.size();k++)
		    {
	    	     ...
	    	     regle+=("\nsource edge= :" + edgrule.getAttributeValue("source")+"---->Tragetedge=  :" 
			     + edgrule.getAttributeValue("target")+"\n");
...
List EdgePac=Rule.getChild("ApplCondition").getChild("PAC").getChild("Graph").getChildren("Edge");
    ...
    regle+=("\nle type de cette ApplCond est :" +Rule.getChild("ApplCondition").getChild("PAC").getChild("Graph").
	    getAttributeValue("kind"));
    regle+=("\nname de cette ApplCond est  :" +Rule.getChild("ApplCondition").getChild("PAC").getChild("Graph").
...
regle+=("\n\t\t on a dans l'Applcond ::::NAC");    
    ...
    regle+=("\nname de cette ApplCond est  :" +Rule.getChild("ApplCondition").getChild("NestedAC").getChild("Graph").
	   getAttributeValue("name"));
